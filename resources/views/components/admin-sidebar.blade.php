@php
    ...
    $isPembinaEkstra = auth()->user()?->role === 'pembina-ekstra';
...
<aside class="admin-sidebar rounded-3xl bg-slate-950 p-8 shadow-2xl text-white h-fit">
    <div class="space-y-8">
        <div>
            <p class="text-xs uppercase tracking-[0.3em] text-slate-600">Panel {{ $isPembinaEkstra ? 'Pembina Ekstra' : 'Admin' }}</p>
            <h2 class="mt-3 text-2xl font-semibold text-slate-900">Quick Access</h2>
            <p class="mt-2 text-sm text-slate-800">
                {{ $isPembinaEkstra ? 'Akses cepat untuk ekstrakurikuler dan prestasi.' : 'Akses monitoring dan CRUD fitur utama dengan cepat.' }}
            </p>
        </div>
