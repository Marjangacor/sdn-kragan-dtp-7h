...
<html lang="id">
    <head>
        ...
        <title>Tambah Ekstrakurikuler | Admin SDN Kragan</title>
    ...
    <body class="admin-page min-h-screen bg-slate-100 text-slate-900">
        <main class="mx-auto max-w-3xl p-6">
            <header class="mb-8 rounded-3xl bg-white p-6 shadow-lg">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-2 text-3xl font-semibold text-slate-900">Tambah Ekstrakurikuler</h1>
                        <p class="mt-2 text-sm text-slate-600">Tambahkan kegiatan baru ke halaman ekstrakurikuler.</p>
                    </div>
            ...
            <form action="{{ route('admin.ekstra.store') }}" method="POST" class="space-y-6 rounded-3xl bg-white p-6 shadow-lg">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <input type="url" name="photo_url" value="{{ old('photo_url') }}" class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-slate-400" />
